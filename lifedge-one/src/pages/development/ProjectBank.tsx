...
import { Construction, Map, TrendingUp, ArrowRight, Building2, MapPin, DollarSign } from 'lucide-react';
...
export default function ProjectBank() {
  ...
  const [rowData] = useState<ProjectRecord[]>([
  ...
  const [colDefs] = useState<ColDef<ProjectRecord>[]>([
    ...
    { field: 'budget', headerName: 'Approved Budget', width: 160, valueFormatter: currencyFormatter, type: 'numericColumn' },
        ...
        >
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-medium text-foreground font-serif flex items-center gap-2">
              <Building2 className="w-5 h-5 text-brand-gold" /> Sector Allocation ($M)
            </h3>
          </div>
          <div className="h-[250px] relative">
            <ResponsiveContainer width="100%" height="100%">
               <BarChart data={mockPSIPData} layout="vertical" margin={{ top: 0, right: 30, left: 10, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke={isDark ? '#333' : '#e5e5e5'} horizontal={false} />
                  <XAxis type="number" stroke={isDark ? '#666' : '#999'} fontSize={10} tickLine={false} axisLine={false} />
                  <YAxis dataKey="name" type="category" stroke={isDark ? '#666' : '#999'} fontSize={10} tickLine={false} axisLine={false} width={80} />
                  <RechartsTooltip 
                  ...
                  />
                  <Bar dataKey="budget" name="Approved Budget" fill="#1E4D2B" radius={[0, 4, 4, 0]} barSize={12} />
                  <Bar dataKey="spent" name="Disbursed" fill="#D4AF37" radius={[0, 4, 4, 0]} barSize={12} />
               </BarChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
